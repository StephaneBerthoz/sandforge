...
Last activity: 2026-05-05 — Phase 04 plan 04-05 shipped in night autopilot. Per-panel-session token budget end to end. TokenBudgetStateSchema + 3 envelopes (ai:budget:state/warn/exceeded). SessionBudget class: increment / preflight / reset, sums all 4 token fields (P-04.6), warn debounced ONCE per session at 80% crossing, exceeded fires every breach + every failing preflight, settingsKey='sandforge.ai.tokenBudgetMaxPerSession' for modal deeplink, NaN guard, percent clamped at 200. Wired into AnthropicAdapter via mutable `budget?: SessionBudget` field — preflight before SDK call (chars/4 heuristic), increment after success. AI_BUDGET_EXCEEDED throws BEFORE entering runWithBreaker so breaker isn't penalised. Setting `sandforge.ai.tokenBudgetMaxPerSession` (default 50000) + EN/FR NLS. services.createSessionBudget(sessionId, broker?) helper. Webview TokenBudgetIndicator (mini bar, green/yellow/red, 4-field tooltip, aria-live). AIChatPanel header subscribes via useMessageListener. Vertical slice: 5 cumulative chats → exactly 1 warn + ≥5 state envelopes; 6th preflight refuses BEFORE SDK invocation. +13 extension tests +6 webview indicator tests. Extension: 4961 → 4974. 310 files / 4974 tests / 0 regressions. Plans 04-01..03 + 04-05 + 04-06 = 41 atomic commits 2cccd43..f9c1ed9.
