...
Last activity: 2026-05-05 — Phase 04 plan 04-02 shipped in night autopilot. AnthropicAdapter wrapped with per-provider CircuitBreaker (3 consecutive failures → open for 5 min) + per-AI-request AbortController (sibling-safe — cancelling one request never aborts another) + EventEmitter re-publishing breaker state-change events. classifyAnthropicError pure helper duck-types SDK errors via dual-signal overloaded check (status===529 OR body.error.type==='overloaded_error'). APIUserAbortError NEVER trips the breaker (P-04.2). Shipped ai:provider:status bridge envelope + AIProviderStatusBanner (FR/EN copy, live mm:ss countdown, `data-testid='ai-provider-status-banner'`, aria-live, defense-in-depth secret-leak grep). 6 i18n keys under `ai.error.*`. AIChatPanel wired via useMessageListener. Vertical slice: 3x529 → open → 4th call zero SDK invocations → vi.advanceTimersByTime(300_001) → next call attempts SDK + breaker re-closes. +24 extension tests (15 errorClassifier + 9 AnthropicAdapter breaker block) +5 webview banner tests. Extension: 4883 → 4907. 305 files / 4907 tests / 0 regressions. Plan 04-01 + 04-02 = 16 atomic commits 2cccd43..e12d0b7.
