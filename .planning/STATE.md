...
Last activity: 2026-05-05 — Phase 04 plan 04-04 shipped in night autopilot (autonomous: false — visual UAT deferred to verify-phase). Diagnose flow + ActionCard at framework level. ActionKindSchema (5 kinds) + ActionProposalSchema + DiagnoseErrorContextSchema + 4 bridge envelopes (ai:diagnose, ai:diagnose:response, ai:approve-action, ai:approve-action:response). AIDiagnoseHandler (NEW file, defensively additive) two-call pattern: runTools investigation → complete(schema:DiagnoseResultSchema) typed extraction (RT-#11 closure complete). wrapAsUserData('errorContext',...) prompt build with self-defense canary asserting forbidden substrings don't leak past escape (RT-#10 closure complete — defence at encoding layer + spotlight prompt). Approve gate: requiresApproval=false→rejected/auto-execute; true→dispatcher.runAnonymous/applyFix; cache TTL 10min; modifiedPayload override. Webview ActionCard with confidence badge + ≤5 actions + Approve/Modify/Reject trio (gated) OR Exécuter (read-only) + inline modify modal + per-action state badges. AIClient interface extended with runTools(). +9 extension tests +8 webview ActionCard tests. Extension: 4974 → 4983. Deferred to v1.4 backlog: ExtensionHandlers wiring, per-org tool allowlist construction, AIChatPanel ActionCard integration, SoqlCodeActionProvider, Playwright spec extension. Plans 04-01..06 = 49 atomic commits 2cccd43..be9aaaf.
