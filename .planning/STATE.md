...
Last activity: 2026-05-05 — Phase 04 plan 04-01 shipped in night autopilot (8 tasks, 8 commits 2cccd43..10d19e4). AI substrate foundation. Bumped zod to ^3.25 + pinned @anthropic-ai/sdk at exact 0.93.0. Shipped `AIClient` interface, `AnthropicAdapter` happy-path (lazy SecretStorage, 4-field usage breakdown, API-key redaction in errors, preserved APIUserAbortError), `AIClientFactory` per-provider memoised (openai/custom throw AINotImplementedError until 04-07), `services.aiClient(provider?)` wired in composition root (lazy — no SecretStorage call at activate). +18 extension tests (9 AnthropicAdapter + 6 AIClientFactory + 3 services.aiClient). Vertical slice: `services.aiClient('anthropic').complete({prompt, schema: DiagnoseResultSchema})` round-trips through mocked SDK to typed payload — RT-#11 closure path opens. Extension: 4865 → 4883. Zero regressions across the full suite (304 test files).
