...
Only to Anthropic, the only provider SandForge offers, and nothing before you turn AI on and store an Anthropic key. From then on, the chat sends your conversation, NL2SOQL sends your question with the names and labels of the org's objects, plus the field API names, labels and types of the up to five objects your question names, a pipeline draft sends your description, a custom persona sends its description, and an AI field rule in Seed, whether you picked it or a built-in persona set it, sends the field's API name and any instruction typed for it. A failed Seed, Sync, DataOps or Automation run also sends its error message automatically, with every 15- or 18-character Salesforce Id in it replaced by `<id>` first, together with the module it came from, the request that started it, the object API names it was working on and, where the handler knows it, the batch size, for a fix suggestion shown as a VS Code notification — unless `sandforge.ai.errorResolution` is off, which stops that sending on its own and leaves the rest of the assistant as it is; unless a built-in table of common Salesforce error codes already answers it, which is decided on your machine, sends nothing and also answers with AI off; unless it is one of the messages SandForge writes itself for a declined production confirmation, a duplicate run, a running backup or rollback, a CRUD, FLS or Production Guard refusal, a missing backup or one taken from another org, a failed automation pipeline with no further detail ("Pipeline failed"), an org that is not found or has no credentials, or a malformed username; and unless no SandForge panel or sidebar view is open. Other errors SandForge raises, such as a failed connection or an invalid request, are still sent. Salesforce error messages can quote record values. Compare's schema advice and Monitor's anomaly scan send nothing. Telemetry is off by default and, turned on, only writes error reports to a local log.
...
Partly. The **Automation** module provides a visual pipeline builder where you compose and save pipelines from 15 step types (seed, sync, backup, restore, anonymize, delete, compare, precheck, script, notification, approval, delay, condition, loop, parallel) and start them by hand. Six of them run: `backup` takes a DataOps snapshot into local storage, `compare` diffs the metadata of two orgs, `precheck` reads an org's API usage, storage, error logs or failed jobs, `notification` shows a VS Code notification, and `delay` and `condition` wait and branch. No pipeline step writes to your org: a pipeline holding `seed`, `sync`, `restore`, `anonymize`, `delete`, `script`, `approval`, `loop` or `parallel` is refused before its first step and recorded as failed. Scheduled and event-driven triggers (cron, webhook, file watch, record change) are not wired yet — see the [Automation guide](modules/automation.md).
...
3. The NL2SOQL helper checks the query only as far as it looked: it describes up to five objects your request names by API name or label, sends those objects' field API names to the model, and rejects a query that selects a plain field none of them has -- a relationship path such as `Account.Name`, and any item carrying a bracket such as `COUNT(Id)`, are left to the org to judge. A subquery such as `(SELECT LastName FROM Contacts)` is not checked either, in the field list or in the WHERE clause: the helper checks the query around it and leaves the subquery's object and fields to the org. When no plain field was compared the draft comes back with a line saying so, and the line says which of the two cases it is: the request named no object the org recognises, so the helper had no field list to check against; or it had one, but the draft selects nothing but related-record paths, subqueries and totals. Run the query and read the error
...
5. The Anomaly Scan button sits in that same panel but reads no limit: it samples records of the object chosen in the dropdown beside it -- Account, one of the 20 largest objects that hold records, or any other object whose API name you type after choosing "Other object…" -- and reports data anomalies (outliers, future dates, negative amounts, bursts of dates less than a minute apart, near-empty fields and duplicate names or emails -- the rules the [Monitor guide](modules/monitor.md) lists), so it will not tell you where your API calls went
