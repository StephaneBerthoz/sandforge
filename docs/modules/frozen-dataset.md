...
2. **Scope-aware extraction**: reuses `RecordScopeCache` and `ScopedSoqlBuilder` (full descendants plus the reference data they need, never `SELECT *`), with a frozen date bound (`CreatedDate <= asOf`) and an `rt-map.json` written to the sas. Discovery runs once per run, up to `maxNodes` objects (default 50); when it stops at that cap, the selection and the manifest **say so**. The dossier grows from the roots through the lookups between records, both ways: the account an opportunity names, the quotes under that account. A lookup the platform will not let a row leave empty also holds the row to the dossier: a quote line whose quote is not in it is left out. One the row may leave empty holds it to nothing -- a quote under the root's account is kept whichever opportunity it belongs to. The **catalog** -- prices, products, selling models, price books -- is read by what the dossier's records name and never walks down into other dossiers: a price book met through a lookup brings none of the quotes or orders priced from it. Its records are fetched by id when they are named late, and whatever `maxNodes` left out of the graph: each line carries its price, and each price its product, its book and its selling model, with the **selling model option** that lets the product be sold under that model, unless `excludedObjects` leaves them out. The **items of an order past Draft** come the same way when discovery stopped before them: the load writes such an order as a draft and activates it once the rest is written, and the platform activates no order without a product, so the extraction reads the items of the orders past Draft in the source -- and only those -- with their prices. A price book keeps one price per product and currency, and per selling model where the dataset carries the models. The **standard prices** of the products in the dataset are read with it, and the standard price book is marked so the load can match it. Objects without `CreatedDate` are read without the date bound and **named**; objects whose files (`base64` fields) the rules do not keep with an approved `keep` are **left out and named** -- bytes cannot be pseudonymized. `excludedObjects` leaves others out.
