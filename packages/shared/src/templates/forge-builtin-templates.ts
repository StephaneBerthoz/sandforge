...
export const BUILTIN_TEMPLATE_OBJECTS: Readonly<Record<string, readonly string[]>> = Object.freeze({
  ...
  // No 'Attachment': a file is not a node. As a node it would query
  // Attachment.Body like any other field and hand it to a writer that routes
  // anything over 200 records through Bulk API 2.0, which rejects base64. The
  // run's files option copies attachments and Salesforce Files in a stage of
  // its own, after the records they hang on, and never through Bulk.
