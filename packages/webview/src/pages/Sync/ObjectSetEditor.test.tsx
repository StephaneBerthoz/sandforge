...
const entries: ObjectSetEntry[] = [
  { objectApiName: 'Account', operation: 'upsert', externalIdField: 'Id', batchSize: 200, where: '' },
  { objectApiName: 'Contact', operation: 'insert', externalIdField: '', batchSize: 200, where: 'IsActive = true' },
];

describe('ObjectSetEditor', () => {
  it('should render the editor', () => {
    render(
      <ObjectSetEditor entries={entries} availableObjects={['Account', 'Contact', 'Lead']} onAdd={vi.fn()} onRemove={vi.fn()} onChange={vi.fn()} />,
    );
  ...
  it('should show existing entries', () => {
    render(
      <ObjectSetEditor entries={entries} availableObjects={['Lead']} onAdd={vi.fn()} onRemove={vi.fn()} onChange={vi.fn()} />,
    );
  ...
  it('should call onRemove when remove is clicked', () => {
    ...
    render(
      <ObjectSetEditor entries={entries} availableObjects={['Lead']} onAdd={vi.fn()} onRemove={onRemove} onChange={vi.fn()} />,
    );
  ...
  it('should show empty state when no entries', () => {
    render(
      <ObjectSetEditor entries={[]} availableObjects={['Account']} onAdd={vi.fn()} onRemove={vi.fn()} onChange={vi.fn()} />,
    );
