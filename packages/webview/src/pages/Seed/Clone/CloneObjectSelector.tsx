...
export const CloneObjectSelector: React.FC<CloneObjectSelectorProps> = ({
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-(--sf-space-2)" data-testid="clone-objects-loading">
  ...
  return (
    <div className="flex flex-col gap-(--sf-space-3)" data-testid="clone-object-selector">
      ...
      >
        {filteredObjects.map((obj) => {
          ...
          return (
            <div key={obj.name} className="flex flex-col">
              <label
                className="flex items-center gap-(--sf-space-2) px-(--sf-space-2) py-(--sf-space-1) rounded-sm cursor-pointer hover:bg-(--sf-bg-hover) text-xs"
              ...
              >
                ...
                />
                <span className="text-(--sf-text-primary) font-medium">{obj.label}</span>
                <span className="text-(--sf-text-primary)">{obj.name}</span>
              </label>
        ...
        {filteredObjects.length === 0 && (
          <span className="text-xs text-(--sf-text-secondary) text-center py-4">
            {t('seed.noObjects')}
          </span>
        )}
      </div>
