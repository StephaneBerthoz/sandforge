...
export interface CardProps extends Omit<React.HTMLAttributes<HTMLDivElement>, MotionEventConflicts> {
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        hoverable && 'cursor-pointer hover:border-[var(--vscode-focusBorder,#007fd4)] transition-colors',
        className,
      )}
      onClick={onClick}
...
export const CardHeader: React.FC<CardHeaderProps> = ({ title, subtitle, action, className }) => {
  return (
    <div className={cn('flex items-center justify-between px-4 py-3 border-b border-[var(--vscode-panel-border,#3c3c3c)]', className)}>
      <div>
        <h3 className="text-sm font-semibold text-[var(--vscode-editor-foreground,#d4d4d4)]">{title}</h3>
        {subtitle && (
          <p className="text-xs text-[var(--vscode-descriptionForeground,#868686)] mt-0.5">{subtitle}</p>
        )}
      </div>
