...
describe('SplitView', () => {
  it('should render left and right content', () => {
    render(<SplitView left={<div>Left content</div>} right={<div>Right content</div>} />);
  ...
  it('should apply custom className', () => {
    render(
      <SplitView left={<div>Left</div>} right={<div>Right</div>} className="my-custom-class" />,
    );
  ...
  it('should apply 50/50 ratio classes', () => {
    render(<SplitView left={<div>Left</div>} right={<div>Right</div>} ratio="50/50" />);
    const leftPanel = screen.getByTestId('splitview-left');
  ...
  it('should apply 70/30 ratio classes', () => {
    render(<SplitView left={<div>Left</div>} right={<div>Right</div>} ratio="70/30" />);
    const leftPanel = screen.getByTestId('splitview-left');
    const rightPanel = screen.getByTestId('splitview-right');
  ...
  it('should call onToggleRight when toggle button is clicked', () => {
    ...
    render(
      <SplitView left={<div>Left</div>} right={<div>Right</div>} onToggleRight={onToggleRight} />,
    );
