...
[![Version](https://img.shields.io/badge/version-1.22.0-blue)](https://marketplace.visualstudio.com/items?itemName=StephaneBerthoz.sandforge)
...
| **AI Assistant**   | With your own Anthropic key: chat, pipeline drafts in Automation, and NL2SOQL, custom personas and AI field rules (picked by you or set by a built-in persona) in Seed. A saved chat can be reread but not continued once VS Code restarts or an AI setting changes. While AI is on, a failed Seed, Sync, DataOps or Automation run sends its error message to the model for a fix suggestion, unless a built-in table of common Salesforce error codes already answers it; turning AI off stops that at once. Compare's schema advice and Monitor's anomaly scan are rule-based: they run with AI off and no key |
