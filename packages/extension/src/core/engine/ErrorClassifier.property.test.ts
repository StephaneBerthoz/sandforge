...
describe('ErrorClassifier — property-based', () => {
  ...
  it('batch classification matches per-element classification', () => {
    fc.assert(
      fc.property(fc.array(salesforceApiErrorArb, { minLength: 0, maxLength: 25 }), (errors) => {
        const classifier = new ErrorClassifier();
  ...
  it('summary totals never lie (totalErrors + retryable/non-retryable + byCategory)', () => {
    fc.assert(
      fc.property(fc.array(salesforceApiErrorArb, { minLength: 0, maxLength: 50 }), (errors) => {
        const classifier = new ErrorClassifier();
        ...
        expect(summary.retryableCount + summary.nonRetryableCount).toBe(errors.length);

        // byCategory covers every error exactly once.
        const byCategorySum = Object.values(summary.byCategory).reduce((a, b) => a + b, 0);
        expect(byCategorySum).toBe(errors.length);

        // byErrorCode total also matches.
        const byCodeSum = Object.values(summary.byErrorCode).reduce((a, b) => a + b, 0);
  ...
  it('byErrorCode maps every input statusCode to its occurrence count', () => {
    fc.assert(
      fc.property(fc.array(salesforceApiErrorArb, { minLength: 0, maxLength: 30 }), (errors) => {
        const classifier = new ErrorClassifier();
        const { summary } = classifier.classifyBatch(errors);

        const expectedCounts: Record<string, number> = {};
        for (const err of errors) {
          expectedCounts[err.statusCode] = (expectedCounts[err.statusCode] ?? 0) + 1;
        }
        ...
        expect(Object.keys(summary.byErrorCode).sort()).toEqual(Object.keys(expectedCounts).sort());
      }),
      { numRuns: 100 },
    );
  });
