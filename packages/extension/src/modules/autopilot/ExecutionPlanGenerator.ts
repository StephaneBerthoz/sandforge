 ...
 * Groups objects into waves based on dependency levels.
...
export class ExecutionPlanGenerator {
   ...
   * In a wave, the objects of one cycle are written one after the other and
   * the others side by side, so a wave lasts as long as its longest cycle or
   * object. Waves are sequential, so durations are summed.
  ...
  private estimateDuration(waves: ExecutionWave[], graph: AutopilotGraph): number {
    ...
    // The cycle each object belongs to; a self-reference is a cycle of one.
    const cycleOf = new Map<string, number>();
    graph.cycles.forEach((cycle, index) => {
      for (const obj of cycle.objects) cycleOf.set(obj, index);
    });

    let totalDuration = 0;

    for (const wave of waves) {
      const callsByGroup = new Map<string, number>();
      for (const obj of wave.objects) {
        const cycle = cycleOf.get(obj);
        const group = cycle === undefined ? `object:${obj}` : `cycle:${cycle}`;
        callsByGroup.set(group, (callsByGroup.get(group) ?? 0) + (apiCallsByObject.get(obj) ?? 0));
      }
      totalDuration += Math.max(0, ...callsByGroup.values()) * this.avgSecondsPerApiCall;
    }
