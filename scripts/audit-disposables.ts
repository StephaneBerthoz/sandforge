...
function main(): void {
  ...
  // Only rewrite the tracked report when the content actually changed, so
  // consecutive runs leave git status clean.
  const previous = existsSync(outPath) ? readFileSync(outPath, 'utf8') : null;
  if (previous !== content) {
    writeFileSync(outPath, content, 'utf8');
    // eslint-disable-next-line no-console
    console.log(`[audit-disposables] report updated: ${outPath}`);
  } else {
    // eslint-disable-next-line no-console
    console.log(`[audit-disposables] report unchanged: ${outPath}`);
  }
  // eslint-disable-next-line no-console
  console.log(
    `[audit-disposables] ${orphans.length} orphan(s) across ${byCategory.size} categories.`,
  );
